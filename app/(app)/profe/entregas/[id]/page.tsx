...
import { analizarExpresion, esGrabacionEntregada } from "@/lib/expresion";
...
export default async function CorregirPage({
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      ...
      {/*
        Quién manda aquí es lo entregado y no la modalidad de la tarea: Pablo
        puede pasar a grabada una escrita que ya tiene la redacción de alguien
        en `entrega`, y entonces preguntarle a la tarea daría un reproductor con
        un texto por `src` y una redacción que no se enseña en ninguna pantalla.
        El audio es privado y la ruta que lo sirve mira la sesión, así que el
        `src` viaja con la cookie del profesor y no hace falta nada más aquí.
        `preload="none"` para no bajar los megas de todas las entregas al abrir
        la pantalla.
      */}
      {registro.entrega && (
        <section className="mt-6 rounded-tarjeta border border-hp-100 bg-white p-6 shadow-suave">
          <p className="text-xs font-bold uppercase tracking-wider text-tinta-suave">
            {esGrabacionEntregada(registro.entrega) ? "Lo que grabó" : "Lo que escribió"}
          </p>
          {esGrabacionEntregada(registro.entrega) ? (
