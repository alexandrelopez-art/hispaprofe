...
].filter((t) => VACIO[t.marca] !== undefined);
...
```tsx
// Ya están los cuatro, así que a partir de aquí `Partial` ya no tapa nada:
// se deja porque `nuevo/page.tsx` filtra por él y un tipo futuro volverá a
// entrar por aquí.
export const VACIO: Partial<Record<MarcaEjercicio, unknown>> = {
  opcion: OPCION_VACIA,
  huecos: HUECOS_VACIO,
